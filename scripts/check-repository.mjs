...
if (practices.length !== 83) throw new Error(`Se esperaban 83 prácticas y existen ${practices.length}`);
...
console.log("Catálogo: 83 prácticas. Diagnóstico: válido.");
...
const guideText = await Promise.all(
  ["01-foundations.md", "02-consensus-bitcoin.md", "03-evm-development.md", "04-professional-security.md",
   "05-advanced-capstone.md", "06-finanzas-onchain.md", "07-data-analytics.md"]
    .map((name) => readFile(join("labs/guides", name), "utf8"))
);
const guidedPractices = guideText.join("\n").match(/^## \d{2} ·/gm) ?? [];
if (guidedPractices.length !== 83) {
  throw new Error(`Se esperaban 83 guías prácticas y existen ${guidedPractices.length}`);
}
console.log("Guías prácticas: 83/83.");
...
if (filasPractica.length !== 83) {
  throw new Error(`El catálogo tiene ${filasPractica.length} prácticas y deberían ser 83.`);
}
const readmeAuto = /(\d+) de las 83 prácticas traen verificación ejecutable/
...
console.log(`Prácticas auto-verificadas: ${practicasAuto}/83, coincide con el README.`);
